...
const LineAnimation = props => {
	useEffect(() => {
		const { id, type = 0, startY = 0, speed, responsive, showFinalActive = false, ballPositions } = props;
		...
		const getBallHTML = (lineItemRect, containerItems, ball, activePoint, finalPoint) => {
			...
			containerItems.forEach((containerItem, containerIndex) => {
				...
				ballItems.forEach(ballItem => {
					...
					if (type === 0 && ballIndex === ballLength) {
						x = finalPoint.x - ball.width / 2;
						y = finalPoint.y - ball.width / 2;
					} else if (type === 0 && ballIndex === 1) {
		...
		const getPathData = (lineItemRect, containerItems, pathWidth, ball) => {
			...
			containerItems.forEach((containerItem, index) => {
				...
				if (containerItems.length > 0) {
					if (type === 0 && index === containerItems.length - 1) {
						if (type === 0) d += ` V ${oldY + 140 - ball.width / 2 - ball.borderWidth / 2}`;
					} else {
						...
						const nextItemRight = (nextItemRect?.right || containerItemRect.right) - lineItemRect.left - ball.width / 2;
						const nextItemLeft = (nextItemRect?.left || containerItemRect.left) - lineItemRect.left + ball.width / 2;
						...
						if (type === 0 && index === containerItems.length - 2) {
						...
						d += ` V`;
						d += ` ${h1}`;
						d += ` C ${c1} ${h2} ${c2} ${oldY} ${c3} ${oldY} H ${c4}`;
		...
		const getSpeed = () => {
			if (responsive) return getResponsive()?.speed || speed || 1;
			else return speed || 1;
		};

		const getPathActiveOffsetLength = pathActive => {
			const totalLength = pathActive.getTotalLength();
			const center = (window.innerHeight * getSpeed()) / 2;
